export default async function handler(req, res) {
  ...
  try {
    ...
    // Step 2: Lấy danh sách ảnh và video trong folder Drive
    ...
    const driveUrl = `https://www.googleapis.com/drive/v3/files?q=${encodeURIComponent(query)}&fields=${encodeURIComponent(fields)}&orderBy=createdTime+asc`;
    ...
    const photos = driveData.files.map((file) => ({
      id: file.id,
      name: file.name.replace(/\.[^/.]+$/, ""), // bỏ đuôi .jpg/.png
      mimeType: file.mimeType,
      thumbnail: file.thumbnailLink,
      // Dùng proxy endpoint để tránh rate limit và auth issues
